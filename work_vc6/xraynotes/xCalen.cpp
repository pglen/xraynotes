...

{
	...
	if(rc.bottom > LARGE_SIZE)
		...
		for(loop = 0; loop < arr[dh][th + starthour * 4 + 1].GetSize(); loop++)
			{
...
{
	//P2N("CxCalen::Set item date='%s' item='%s'\r\n", ddd->Format("%c"), ste);
...

{
	...
	if(day < 0)
		{
		P2N("Day is too small or negative\r\n");
		nonok = true;
		}
	if(hour < 0)
		{
		P2N("HOUR is too small or negative\r\n");
		nonok = true;
		}
	...
	for(int loop = 0; loop < WEEKDAYS; loop++)
		...
		for(int loop2 = 0; loop2 < SHOWHOURS; loop2++)
			...
			if(rc.bottom > LARGE_SIZE)
				...
				{
				if(	*data[loop][(loop2 + starthour) * 4 + 0] != "" ||  
					*data[loop][(loop2 + starthour) * 4 + 1] != "")
					{
					//P2N("Data %d %d -- %s\r\n", loop, loop2  + starthour, *data[loop][(loop2 + starthour) * 2]);
