...
/* =====[ parse.cpp ]========================================== 
   ...
   Description:     The dbtest project, implementation of the parse.cpp                
   ...
   Revisions:                                                                
      ...
      ----  --------  -----------  ----------------------------   
      0.00  1/14/2008  Peter Glen   Initial version.                         
...
//
// Function name:    
// Description:      Parse strings into a string array
...
int		ParseStrToArray(const char *cmdline, const char *delim, CString *argarr, int limit)

{
	...
	for(loop = 0; loop < len; loop++)
		...
		if(strchr(delim, chh) != NULL)
			{
			// if prev was not a delim
			if(strchr(delim, prev) == NULL)
				{
				idx++;
				}			
			}
