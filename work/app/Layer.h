...
public:
	...
	// Overridden methods
	virtual int		Invalidate(int dummy1 = 0, int dummy2 = 0, int dummy3 = 0);
	virtual int		HitTest(POINT pt, int dummy1 = 0, int dummy2 = 0);
	virtual int		Prop(int dummy1, int dummy2, int dummy3);
	virtual int		Init(POINT pt, int dummy1 = 0, int dummy2 = 0);
	virtual int		KeyEvent(UINT event, UINT nFlags, UINT nChar);
	virtual int		MouseEvent(UINT event,  UINT nFlags, CPoint point);
	virtual int		Draw(CDC* pDC, float factor, int dummy1 = 0);
...

{
	...
	typedef enum _xfunc
	{
		XFUNC_DRAW, XFUNC_INIT, XFUNC_MOUSE, XFUNC_KEY, XFUNC_PROP, XFUNC_INVAL, XFUNC_HIT
	} xfunc;
