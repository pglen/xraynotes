...

{
	...
	if(!IsWindow(m_hWnd))
		...
		SetWindowPos( NULL, want_x, want_y, ww, hh,  SWP_NOZORDER); 	
		
		CLabel *lab =  (CLabel*)GetDlgItem(IDC_PROMPT);
		ASSERT(lab);
		WINDOWPLACEMENT wp;
		lab->GetWindowPlacement(&wp);
		int www = wp.rcNormalPosition.right - wp.rcNormalPosition.left;
		int hhh = wp.rcNormalPosition.bottom - wp.rcNormalPosition.top;

		lab->SetWindowPos( NULL, ww/2 - www / 2, hh/2 - hhh/2 , 0, 0,  SWP_NOSIZE | SWP_NOZORDER ); 			
		}
