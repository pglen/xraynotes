@echo off
REM -- First make map file from Microsoft Visual C++ generated resource.h
echo // MAKEHELP.BAT generated Help Map file.  Used by UNICODE.HPJ. >"hlp\unicode.hm"
echo. >>"hlp\unicode.hm"
echo // Commands (ID_* and IDM_*) >>"hlp\unicode.hm"
makehm ID_,HID_,0x10000 IDM_,HIDM_,0x10000 resource.h >>"hlp\unicode.hm"
echo. >>"hlp\unicode.hm"
echo // Prompts (IDP_*) >>"hlp\unicode.hm"
makehm IDP_,HIDP_,0x30000 resource.h >>"hlp\unicode.hm"
echo. >>"hlp\unicode.hm"
echo // Resources (IDR_*) >>"hlp\unicode.hm"
makehm IDR_,HIDR_,0x20000 resource.h >>"hlp\unicode.hm"
echo. >>"hlp\unicode.hm"
echo // Dialogs (IDD_*) >>"hlp\unicode.hm"
makehm IDD_,HIDD_,0x20000 resource.h >>"hlp\unicode.hm"
echo. >>"hlp\unicode.hm"
echo // Frame Controls (IDW_*) >>"hlp\unicode.hm"
makehm IDW_,HIDW_,0x50000 resource.h >>"hlp\unicode.hm"
REM -- Make help for Project UNICODE


echo Building Win32 Help files
start /wait hcw /C /E /M "hlp\unicode.hpj"
if errorlevel 1 goto :Error
if not exist "hlp\unicode.hlp" goto :Error
if not exist "hlp\unicode.cnt" goto :Error
echo.
if exist Debug\nul copy "hlp\unicode.hlp" Debug
if exist Debug\nul copy "hlp\unicode.cnt" Debug
if exist Release\nul copy "hlp\unicode.hlp" Release
if exist Release\nul copy "hlp\unicode.cnt" Release
echo.
goto :done

:Error
echo hlp\unicode.hpj(1) : error: Problem encountered creating help file

:done
echo.
