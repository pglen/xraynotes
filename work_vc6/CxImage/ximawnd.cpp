...
{
	return DrawX(hdc, x, y, cx, cy, pClipRect, bSmooth, 0, 0);
}

long 	CxImage::DrawX(HDC hdc, long x, long y, long cx, long cy, RECT* pClipRect, bool bSmooth, 
						int xoffset, int yoffset)

{
	//P2N("DrawXnew x=%d y=%d cx=%d cy=%d xoffset=%d yoffset=%d\r\n", x, y, cx, cy, xoffset, yoffset);
	...
	if (!(bTransparent || bAlpha || info.bAlphaPaletteEnabled))
		...
		if (cx==head.biWidth && cy==head.biHeight)
			{ //NORMAL
			//P2N("Normal GDI rendering\r\n");

			SetStretchBltMode(hdc, COLORONCOLOR);
			SetDIBitsToDevice(hdc, x, y, cx, cy, xoffset, 0, yoffset, cy,
						info.pImage,(BITMAPINFO*)pDib, DIB_RGB_COLORS);
			}
		 else
		 	{ //STRETCH
			//P2N("Streched GDI rendering\r\n");
			//pixel informations
			RGBQUAD c={0,0,0,0};
			//Preparing Bitmap Info
			BITMAPINFO bmInfo;
			memset(&bmInfo.bmiHeader, 0x00, sizeof(BITMAPINFOHEADER));
			...
			if (pbase)
				...
				for(yy=0; yy < desth; yy++)
					{
					dy = head.biHeight-(ymax-yy-y+yoffset)*fy;
					sy = max(0L,(long)floor(dy));
					psrc = info.pImage + (sy) * info.dwEffWidth ; //+ int(yoffset * fy * info.dwEffWidth);
					pdst = pbase+yy*ew;

					//////////////////////////////////////////////////////
					//for(xx = 0; xx < destw; xx++)
					for(xx = xoffset; xx < destw + xoffset; xx++)
						{
						dx = (xx + xmin - x) * fx;
			...
			SetDIBitsToDevice(hdc, paintbox. left, paintbox.top, destw, desth, 
									0, 0, 0, desth, pbase, &bmInfo, 0);
	...
	// Draw image with transparent/alpha blending
		...
		{
		//P2N("Alpha blend rendering\r\n");

		//////////////////////////////////////////////////////////////////
		// Alpha blend - Thanks to Florian Egel
		// Dynamic alpha blend - Thanks to Peter Glen
		...
		// Nope, no bitmap, clean up and bail
		if (!pbase)
			{
			//P2N("No bitmap on Draw\r\n");
			DeleteObject(SelectObject(TmpDC, TmpObj));
			DeleteDC(TmpDC);
			return true;
			}
		...
		if (cx!=head.biWidth || cy!=head.biHeight)
			...
			for(yy=0;yy<desth;yy++){
				...
				{
				//NORMAL
				//P2N("Alpha, normal rendering trans_tresh=%d alphamax=%d transflag=%d\r\n",
				//				   info.trans_tresh, info.nAlphaMax, bTransparent );
					...
					for(xx=0;xx<destw;xx++,ix++)
						...
						if ((head.biClrUsed && ci!=cit) || (!head.biClrUsed && *pc!=*pct) || !bTransparent)
							...
							if (a == 0)
								...
								{	// opaque, ignore dest
								if( c.rgbBlue  > info.trans_tresh &&
									c.rgbGreen > info.trans_tresh &&
									c.rgbRed   > info.trans_tresh)
									{
									pdst+=3;
									}
								else
								...
								{	// semi transparent
//define DYNAMIC_BLENDING

#ifdef  DYNAMIC_BLENDING
								// Dynamic alpha blending
								if( c.rgbBlue  > info.trans_tresh &&
									c.rgbGreen > info.trans_tresh &&
									c.rgbRed   > info.trans_tresh)
									{
									int delta = (255 - info.trans_tresh) + 1;

									BYTE a2 =  (255 - ((int)c.rgbBlue +
												(int)c.rgbGreen +
													 (int)c.rgbRed ) / 3) ;
									a2 = ((int)a2 * 255) / delta;
									...
									*pdst++=(BYTE)((*pdst * a3 + a2 * c.rgbBlue)  >> 8);
									*pdst++=(BYTE)((*pdst * a3 + a2 * c.rgbGreen) >> 8);
									*pdst++=(BYTE)((*pdst * a3 + a2 * c.rgbRed)   >> 8);
									}
#else
								//if( c.rgbBlue  > info.trans_tresh &&
								//	c.rgbGreen > info.trans_tresh &&
...
//								else
		...
		// Paint the image & cleanup
		SetDIBitsToDevice(hdc, paintbox.left, paintbox.top, destw,
										desth, xoffset, 0, yoffset, desth, pbase, &bmInfo, 0);
		DeleteObject(SelectObject(TmpDC,TmpObj));
		DeleteDC(TmpDC);
	}
...
CxImage	*CxImage::DrawString2(HDC hdc, long x, long y, CXTEXTINFO *pTextType, bool bSetAlpha )

{
	if (!IsValid())
        return NULL;

	long	org_x = x, org_y = y;
	...
	CxImage itext;
	CxImage *xtext = new CxImage(width, height, 24);
    ...
    if ( pTextType->opaque )
    {
        ...
        for (ix=0;ix<width;ix++)
        {
            ...
            for (iy=0;iy<height;iy++)
				{
				RGBQUAD bcolor, pcolor;
				pcolor = GetPixelColor(org_x+ix, y + iy - height);

                if ( (ix<=roundR && ( iy > height-ixf-1 || iy < ixf )) ||
                     (ix>=width-roundR-1 && ( iy > height-ixf-1 || iy < ixf )) )
					{
					xtext->SetPixelColor(ix, iy, pcolor,bSetAlpha);
					continue;
					}
                else
					{
                    if ( pTextType->b_opacity > 0.0 && pTextType->b_opacity <= 1.0 )
                        ...
                        bcolor.rgbGreen = (unsigned char)(pTextType->b_opacity * pcolor.rgbGreen + (1.0-pTextType->b_opacity) * p_backcolor.rgbGreen ) ;
                        bcolor.rgbReserved = 0;
						xtext->SetPixelColor(ix, iy, bcolor, bSetAlpha);
						}
					}
			}
		}
    ...
    for (ix=0;ix<width;ix++)
    {
		for (iy=0;iy<height;iy++)
        {
            if (itext.GetPixelColor(ix,iy).rgbBlue )
				{
                xtext->SetPixelColor(ix+frame,iy-frame,p_forecolor,bSetAlpha);
				}
		}
	}
...
void 	CxImage::InitTextInfo( CXTEXTINFO *txt )
{
    ...
    // the text
    _stprintf( txt->text, _T("Sample Text")); // text use TCHAR mappings <Cesar M>
    txt->align = DT_CENTER;
    return;
}
////////////////////////////////////////////////////////////////////////////////
