...
\table [tname]
    ...
    (The column break replaces the double space) The line can be continued if
...
def main ():
    ...
    global currdir, defpage, defcontext, deftable, cppdir
...
def dofile(fname):
    ...
    try:
        for line in ff:
            ...
            if idx >= 0:
                # filter out //----
                if len(line) > idx + len(marker) + 1:
                    if line[idx + len(marker) + 1 ] != "-":
                        # Collect logical lines
                        line = line.strip()
                        idx = line.find(marker)
                        if line.endswith("\\"):
                            #print "logical line", line
                            coll += line[idx + len(marker):len(line)-1]
                        else:
                            parseline (coll + line[idx + len(marker):])
                            coll = ""
    finally:
        ff.close()
...
def  emitfiles():
    ...
    for dd in tree.folderlist(root):
        ...
        if coll == 0:
            hhh.write("No contents found for this file.<p>")

            if dd.name() == "misc.html":
                hhh.write("This should happen if no dangling "
                          "(uncategorized) data found.<br>")
            else:
                hhh.write("This could happen if an unenclosed table "
                      "accumulates all text eating the output statement.<br>")                
        hhh.write(foot)
        hhh.close
