...

{
	...
	if(aszOutString) free(aszOutString);
}
...
int     CxPad::printf(const char *Format, ...)
...
int     CxPad::aprintf(const char *Format, ...)
...
int     CxPad::dump(const void *mem, int len)
...
CxPad::SetLocal(int flag)

{
	local = flag;
}

CxPad::SetRemote(int flag)
...
int     CxPad::dumpmx(const char *dptr, int len)
...
DWORD   CxPad::UnicodeToAnsi(LPWSTR pszW, LPSTR* ppszA)
{
...
DWORD   CxPad::AnsiToUnicode(LPCSTR pszA, LPWSTR* ppszW)
...
int     CxPad::is_nonclient(int msg)
...

{
	...
	ret_val = sendto(sock, "CxPad", 6, 0, (struct sockaddr *)&local_sin, acc_sin_len);
