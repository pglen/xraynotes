...

{
	...
	if(!noheader)
		...
		// Draw Header
		CRect rcc(titlerect); rcc.top += 2; rcc.left += 3;
...

{
	...
	if(pMsg->message == WM_MOUSELEAVE)
		{
		//P2N("CGrayDlg::Mouse leave message\r\n");
		//tip.Hide(); capped = fired = false;
...

{
	...
	in_close = PtInRect(&closerect, point);
	if(in_close != old_in_close)
		{	
		//P2N("CGrayDlg::OnMouseMove in header\r\n");
		old_in_close = in_close;
		InvalidateRect(&closerect);
		}
...
void CGrayDlg::OnMouseLeave()

{
	//P2N("CGrayDlg::OnMouseLeave() %p\r\n", this);

	if(in_close)
		{
		//P2N("Forced head closse\r\n");
		old_in_close = in_close = 0;
		InvalidateRect(&closerect);
		}
}
...
void CGrayDlg::OnMouseEnter()
{
	//P2N("CGrayDlg::OnMouseEnter() %p\r\n", this);
}

// Virtual function to ..

void CGrayDlg::OnMouseHover()

{
	//P2N("CGrayDlg::OnMouseHover()\r\n");
}
