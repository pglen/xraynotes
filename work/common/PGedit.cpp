...
static CString userdicname ("userspell.txt");
static CString maindicname ("spell.txt");
static CString meddicname  ("medspell.txt");
...
static CString approot, dataroot, appdataroot;
...

{
	...
	// These are static, will be fetched ONCE
	if(approot == "")
		{
	...
	if(appdataroot == "")
		{
		support.GetAppDataDir(appdataroot);
		}	
...

{
	...
	dicmain = dataroot; dicmain +=  maindicname;    
	dicidx  = appdataroot; dicidx +=  "spell.idx";

	//P2N("CPGedit::zInit_Spellcheck MAIN Dictionary: %s\r\n", dicmain);
	//P2N("CPGedit::zInit_Spellcheck MAIN index: %s\r\n", dicidx);
	
	dicmed	= dataroot; dicmed += meddicname;
	dicmedidx = appdataroot; dicmedidx += "medspell.idx";

	//P2N("CPGedit::zInit_Spellcheck MED Dictionary: %s\r\n", dicmed);
	//P2N("CPGedit::zInit_Spellcheck MED index: %s\r\n", dicmedidx);

	dicuser = appdataroot; dicuser +=  userdicname;
    ...
    if(_access(dicuser, 02) < 0)
        ...
        if(!fp3)
            {
			AfxMessageBox("Cannot open/create user dictionary file\n");
			return;
			}
		else
			{
    ...
    if(!fp1)
        {
        AfxMessageBox("Internal: Cannot open dictionary data file\n");
        }
    ...
    if(!fp2)
        {
        AfxMessageBox("Internal: Cannot open med dictionary data file\n");
        }
    
	fp3 = fopen(dicuser, "r+t");
    if(!fp3)
        {
		AfxMessageBox("Internal: Cannot open/create user dictionary file\n");
		}
}
...

{
	...
	if(crflag)
        ...
        if(!fp1a)
            {
            AfxMessageBox("Cannot open dictionary data file\n");
            return 0;
            }
        ...
        if(!fp2b)
            {
  			CString idxn2  = appdataroot; idxn2 +=  idxname;
			...
			if(!fp2b)
				{
				AfxMessageBox("Cannot create dictionary index file\n");
				return 0;
				}	
            }
        ...
        while(TRUE)
			...
			if(count % 10000 == 0)
				{
				CString str; str.Format("Indexing dictionary '%s' (%d)\r\nPlease wait ...", fname, count);
				SetWindowText(str);
...

{
	...
	if(!fp2z)
        {
        AfxMessageBox("Internal: Cannot open dictionary index file\n");
		return;
		}
...
int CPGedit::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
