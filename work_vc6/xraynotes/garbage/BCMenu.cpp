...
{
	//P2N("Drawitem XP %s\r\n", ((BCMenuData*)(lpDIS->itemData))->GetString());
...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	COLORREF m_clrBack=GetSysColor(COLOR_WINDOW);
		...
		// XrayNotes has a darker Color
	m_clrBack=DarkenColor(m_clrBack,0.4);
	...
	// XrayNotes has a darker Color
	m_newclrBack=DarkenColor(m_newclrBack,0.4);
	...
	if(state & MF_SEPARATOR){
		...
		if(!IsWinXPLuna())crSelectFill=LightenColor(crSelect,0.4);
		else crSelectFill=LightenColor(crSelect,0.5);
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			if(checkflag && checkmaps){
				...
				if(state&ODS_CHECKED){
					CBrush brushin;
					brushin.CreateSolidBrush(LightenColor(crSelect,0.3));
		...
		if(!strText.IsEmpty()){
			...
			if(!(lpDIS->itemState & ODS_GRAYED)){
				...
				else pDC->SetTextColor(LightenColor(GetSysColor(COLOR_GRAYTEXT), 0.5));
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	//P2N("measureitem: '%s'\r\n", ((BCMenuData*)(lpMIS->itemData))->GetString());
...
#define DECL_STRUCT(MenuItemInfo)					\
                           ...
                           int nIconNormal)
{
	...
	// Owner draw, all the time
	nFlags |= MF_OWNERDRAW;

	mdata->nFlags = nFlags;
	mdata->nID = nID;

	//P2N("Appendenu: '%s' flag=%d id=%d data=%p\r\n", mdata->GetString(), nFlags, nID, mdata);

	int returnflag = 0;
	if(nFlags & MF_SEPARATOR)
		{
		returnflag |=  CMenu::AppendMenu(nFlags, nID, (LPCTSTR)mdata);
		}
	else
		{
		// Switched to two stage shadow setting, so there is a string 
		
		returnflag =  CMenu::AppendMenu(nFlags & ~MF_OWNERDRAW, nID, mdata->GetString());

		//returnflag |= CMenu::AppendMenu(nFlags, nID, (LPCTSTR)mdata);
		//returnflag |= CMenu::ModifyMenu(nID,  MF_STRING, nID, (LPCTSTR)mdata->GetString());

		returnflag = ModifyMenu(GetMenuPos(nID), nFlags | MF_BYPOSITION | MF_OWNERDRAW, (UINT)nID, (LPCTSTR)mdata);
		...
		GetMenuItemInfo(nID, &MenuItemInfo);

		//MenuItemInfo.fMask = MIIM_DATA | MIIM_TYPE;
		//MenuItemInfo.fMask = MIIM_TYPE;
		MIIM_CHECKMARKS | MIIM_DATA | MIIM_ID | MIIM_STATE | MIIM_SUBMENU | MIIM_TYPE ;

		//MenuItemInfo.dwItemData  = (DWORD)mdata;
		MenuItemInfo.fType |= MFT_STRING;

		CString str = mdata->GetString();
		MenuItemInfo.dwTypeData = (LPTSTR)str.LockBuffer();
		MenuItemInfo.cch = mdata->GetString().GetLength();

		int ret;
		ret = SetMenuItemInfo(m_hMenu, nID, false, &MenuItemInfo);
		P2N("SetMenuItemInfo ret=%d err=%d str=%s\r\n", ret, GetLastError(), support.Err2Str());
...
BOOL BCMenu::ModifyMenuFlag(UINT flagadd, UINT flagrem, UINT nID)

{
	...
	for(ii = 0; ii <= m_MenuList.GetUpperBound(); ii++)
		{
		//P2N("flagMenu searched: '%s'\r\n", m_MenuList[ii]->GetString());

		if (m_MenuList[ii]->nID == nID)
			{
			pData = m_MenuList[ii];
			break;
			}
	...
	if (pData)
		{
		pData->nFlags |= flagadd;
		//returnflag = ModifyMenu(GetMenuPos(nID), MF_BYPOSITION | MF_OWNERDRAW, (UINT)nID, (LPCTSTR)mdata);
		ret =  CMenu::ModifyMenu(GetMenuPos(nID), (MF_BYPOSITION | flagadd), nID, (LPCTSTR)0);
		//CMenu::ModifyMenu(GetMenuPos(nID), MF_BYPOSITION | MF_OWNERDRAW, (UINT)nID, (LPCTSTR)pData);
		}

	return ret;
}

BOOL BCMenu::ModifyMenuStr(const char *lpstrText, UINT nID)

{
	...
	if (pData)
		{
		pData->SetWideString(A2W(lpstrText));
		ret =  CMenu::ModifyMenu(nID, MF_STRING | MF_BYCOMMAND, nID, (LPCTSTR)lpstrText);
		}
...

{
	...
	do{
		if(psubmenu && nLoc>=0)mdata = psubmenu->m_MenuList[nLoc];
			...
			P2N("Creating new menu structt\r\n");
			mdata = new BCMenuData;
			m_MenuList.Add(mdata);
		}
		
		P2N("Find old menu data: %s\r\n", mdata->GetString());
	...
	P2N("Modify: '%s'\r\n", (LPCTSTR)mdata->GetString());

	//ret =  CMenu::ModifyMenu(nID, MF_STRING | MF_BYCOMMAND, nID, (LPCTSTR)mdata->GetString());

	ret |= CMenu::ModifyMenu(nID, mdata->nFlags, nID,(LPCTSTR)mdata);
...

{
	...
	CPen pen; pen.CreatePen(PS_SOLID,0,color);
...

{
	...
	CPen m_penBack;	m_penBack.CreatePen (PS_SOLID,0,color);
	CPen *pOldPen = pDC->SelectObject (&m_penBack);

	pDC->MoveTo(x,y+2);		pDC->LineTo(x,y+5-dp);
	pDC->MoveTo(x+1,y+3);	pDC->LineTo(x+1,y+6-dp);
	pDC->MoveTo(x+2,y+4);	pDC->LineTo(x+2,y+7-dp);
	pDC->MoveTo(x+3,y+3);	pDC->LineTo(x+3,y+6-dp);
	pDC->MoveTo(x+4,y+2); 	pDC->LineTo(x+4,y+5-dp);
	pDC->MoveTo(x+5,y+1); 	pDC->LineTo(x+5,y+4-dp);
	pDC->MoveTo(x+6,y); 	pDC->LineTo(x+6,y+3-dp);
...
{
	//char *str = NULL;
	//mxpad.UnicodeToAnsi(lpstrText, &str);
	//P2N("AppendODPopupMenuW: '%s'\r\n", str);
	//free (str);
	...
	//P2N("created submenu %d %d\r\n", pSubMenu, pSubMenu->m_hMenu);
...
//////////////////////////////////////////////////////////////////////////
// Get Menu position from ID

DWORD BCMenu::GetMenuPos(DWORD nID)

{
	for(DWORD loop = 0; loop < GetMenuItemCount(); loop++)
		{
		if(GetMenuItemID(loop) == nID)
			return loop;
		}
