	...
	: CGrayDlg(CXrayM::IDD, pParent)
{
	...
	// Set colors
	dlgbackcol	 = RGB(100, 100, 100); //((CShadesApp*)AfxGetApp())->dlgbackcol;
...

{
	...
	if(hRsrc == NULL)
		{	
		AfxMessageBox("Could not open menu resource."); //, lpszResourceName);
		return FALSE;
		}
	...
	if(hGlobal == NULL)
		{
		AfxMessageBox("Could not load menu resource"); // %d", lpszResourceName);
		return FALSE;
		}	
...

{
	...
	CPen	pen(PS_SOLID,1,dlgbackcol);
	CBrush	brush(dlgbackcol);

	//CPen	*oldpen = (CPen*)pDC->SelectObject(pen);	
	//CBrush	*oldbrush = (CBrush*)pDC->SelectObject(brush);

	pDC->SelectObject(pen);	pDC->SelectObject(brush);
	pDC->Rectangle(&rc);
