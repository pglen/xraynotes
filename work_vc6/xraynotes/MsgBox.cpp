...

{
	...
	// Autoclose
	int mt =  m_timer_str.GetLength();
	if (mt)
		{
		LOGFONT lf;	m_font.GetLogFont(&lf);

		WINDOWPLACEMENT wp;			
		GetDlgItem(IDOK)->GetWindowPlacement(&wp);
				
		RECT rc5 = rc; rc5.top = wp.rcNormalPosition.top - (abs(lf.lfHeight) + 5);
		dc.DrawText(m_timer_str, mt, &rc5, DT_CENTER);
		}
...
{	
	...
	if(tocount > AUTOWARN)
		{
		m_timer_str.Format("AutoClosing Dialog in %d sec", (timeout + 2) - tocount);
