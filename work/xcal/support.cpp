...

{
	...
	if(_access(adata, 0) < 0)
		{
		MBOX("Cannot access application data storage, switching to c:\\tmp\\");
		adata = "c:\\tmp\\";
		}
}

void	CSupport::GetSharedDataDir(CString &sdata)
...
{	
	...
	if(_access(sdata, 0) < 0)
		{
		MBOX("Cannot access application common data storage.");
		//sdata = "c:\\tmp\\";
		}
}
...
{	
...
#if 0
	char *tmp = ttt.GetBuffer(MAX_PATH + 1); ASSERT(tmp);
	GetEnvironmentVariable("ALLUSERSPROFILE", tmp, _MAX_PATH);
	ttt.ReleaseBuffer();
	...
	if(_access(ttt, 0) < 0)
		{
		MBOX("Cannot access application common document storage.");
		//sdata = "c:\\tmp\\";
		}
...
void	CSupport::SplitPath(CString &full,  CString &drive,
