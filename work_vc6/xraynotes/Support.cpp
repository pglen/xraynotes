...
{	
	...
	if(_access(sdata, 0) < 0)
		{
		xMsgBox("Cannot access application common data storage, switching.");
		//sdata = "c:\\tmp\\";
		}
}
...
#ifdef _DEBUG
	str = approot + "..\\..\\install\\umanual\\" + file;
#else
	str =  approot + "umanual\\" + file;
#endif

	int ret = (int)ShellExecute(NULL, "open", str, NULL, NULL, SW_SHOWMAXIMIZED);
	if(ret <= 32)
		{
		P2N("Cannot open help %s\r\n", str);
		ShowStatus("Cannot open help ...");
		}
}
...
{	
	...
	if(_access(ttt, 0) < 0)
		{
		xMsgBox("Cannot access application common document storage.");
		//sdata = "c:\\tmp\\";
		}
	sdata = ttt;
#endif
