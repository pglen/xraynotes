...
BEGIN_MESSAGE_MAP(CShadesDlg, CDialog)
	...
	ON_WM_MOVE()
	ON_COMMAND(ID_EXITALT_RESETDIALOGPOSITIONS, OnExitaltResetdialogpositions)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

//////////////////////////////////////////////////////////////////////////
// Macros to configure XNT dialogs
// We use dialog ID instead of name

#define CONFIGDLG(flag, win, wname, wantx, wanty, wantw, wanth)	\
...

{
	...
	m_hIcon = AfxGetApp()->LoadIcon(IDI_MAINICON);	ASSERT(m_hIcon);
    ...
    SetWindowText("XrayNotes");

    nwfo.parent = this;  nwfo.bitmap = IDB_BITMAP17;
    nefo.parent = this;  nefo.bitmap = IDB_BITMAP19;
    swfo.parent = this;  swfo.bitmap = IDB_BITMAP16;
    sefo.parent = this;  sefo.bitmap = IDB_BITMAP23;
	...
	targcircle = AfxGetApp()->LoadCursor(IDC_CURSOR1);
    ASSERT(targcircle);

    pencil = AfxGetApp()->LoadCursor(IDC_CURSOR2);
    ASSERT(pencil);

    selectcur = AfxGetApp()->LoadCursor(IDC_CURSOR3);
    ASSERT(selectcur);

    markcur = AfxGetApp()->LoadCursor(IDC_CURSOR4);
    ASSERT(markcur);

	rectcur = AfxGetApp()->LoadCursor(IDC_CURSOR9);
	ASSERT(rectcur);

    circcur = AfxGetApp()->LoadCursor(IDC_CURSOR10);
	ASSERT(circcur);

    textcur = AfxGetApp()->LoadCursor(IDC_CURSOR11);
	ASSERT(textcur);

    linecur = AfxGetApp()->LoadCursor(IDC_CURSOR12);
	ASSERT(linecur);

	imgcur = AfxGetApp()->LoadCursor(IDC_CURSOR13);
	ASSERT(imgcur);
...

{
    ...
    for(int loop4 = 0; loop4 < ndoc; loop4++)
        {
...

{
	...
	if(((CShadesApp*)AfxGetApp())->inmodal > 0 ||
        ...
        xMsgBox("Cannot exit, a modal dialog is still up.", "Exiting XrayNotes");
        return;
        }
...

{
    ...
    if(stricmp(lpszSection, "Windows") == 0 || stricmp(lpszSection, "Desktop") == 0)
        {
...
{
	nwfo.arr_l[0].func = functs->OpenFileCb;
    nwfo.arr_l[1].func = functs->ScanDocCb;
    nwfo.arr_l[2].func = functs->CloseFileCb;
    nwfo.arr_l[3].func = functs->SaveFileCb;
    nwfo.arr_l[4].func = functs->SaveAsCb;
    nwfo.arr_l[5].func = functs->PrintCb;
    nwfo.arr_l[6].func = functs->CopyCb;
    nwfo.arr_l[7].func = functs->CutCb;
    nwfo.arr_l[8].func = functs->PasteCb;
    //nwfo.arr_l[9].func = functs->TestCb;
	nwfo.arr_l[9].func = functs->ExitCb;
	nwfo.arr_r[9].func = functs->rExitCb;

	nwfo.arr_r[0].func = functs->rOpenFileCb;
	nwfo.arr_r[2].func = functs->rCloseFileCb;
	nwfo.arr_r[6].func = functs->rCopyCb;
    nwfo.arr_r[8].func = functs->rPasteCb;

	// -------------------------------------------------------------------

	swfo.arr_l[0].func = functs->CropCb;
    swfo.arr_r[0].func = functs->rCropCb;
    swfo.arr_l[1].func = functs->NormCb;
    swfo.arr_l[2].func = functs->BrightCb;
    swfo.arr_l[3].func = functs->ColorCb;
    swfo.arr_l[4].func = functs->MagCb;
    swfo.arr_l[5].func = functs->ProcCb;
    swfo.arr_l[6].func = functs->UndoCb;
    swfo.arr_l[7].func = functs->RedoCb;
    swfo.arr_l[8].func = functs->SelectCb;
    swfo.arr_l[9].func = functs->MarkCb;

	swfo.arr_r[5].func = functs->rProcCb;
    swfo.arr_r[4].func = functs->rMagCb;

	// -------------------------------------------------------------------
	
	nefo.arr_l[0].func = functs->ListDocsCb;
    nefo.arr_l[1].func = functs->DocPropCb;
    nefo.arr_l[2].func = functs->ToggleUICb;
    nefo.arr_l[3].func = functs->ConfCb;
    nefo.arr_l[4].func = functs->OptCb;
    nefo.arr_l[5].func = functs->AutoCb;
    nefo.arr_l[6].func = functs->PatCb;
    nefo.arr_l[7].func = functs->cNewCb;
    nefo.arr_l[8].func = functs->NewCb;
    nefo.arr_l[9].func = functs->ExitCb;
	nefo.arr_r[9].func = functs->rExitCb;

	// -------------------------------------------------------------------
	
	sefo.arr_l[0].func = functs->LineCb;
    sefo.arr_l[1].func = functs->DrawCb;
    sefo.arr_l[2].func = functs->TextCb;
    sefo.arr_l[3].func = functs->RectCb;
    sefo.arr_l[4].func = functs->EllipseCb;
    sefo.arr_l[5].func = functs->ImageCb;
    sefo.arr_l[6].func = functs->SVGCb;
    sefo.arr_l[7].func = functs->MagDocCb;
    sefo.arr_l[8].func = functs->MeasCb;
    sefo.arr_l[9].func = functs->IdleCb;

    sefo.arr_r[2].func = functs->rTextCb;
}
...

{
	...
	// Set them
	for(looplay = 0; looplay < lsize; looplay++)
		{
		if( ((CLayer*)doc->m_layers[looplay])->focus)
			{
...
//////////////////////////////////////////////////////////////////////////
// Dialog Windows initial configuration and reset configuration

void CShadesDlg::InitPos(int flag) 
{	
	RECT rc; GetClientRect(&rc);
	//CWnd

	// CONFIGDLG(flag, win, wname, wantx, wanty, wantw, wanth)	

	CONFIGDLG(flag, dicomwin, "dicomwin", rc.right - 300, rc.top + 70, -1, -1);
	CONFIGDLG(flag, magdlg, "magdlg", rc.left + 70, rc.bottom - 122, -1, -1);
	CONFIGDLG(flag, pydlg, "pydlg", rc.left + 70, 100, -1, -1);
	CONFIGDLG(flag, gesdlg, "gesdlg", rc.left + 70, rc.bottom - 440 , -1, -1)	
	CONFIGDLG(flag, xyzdlg, "xyzdlg", rc.left + 70, rc.top + 150, -1, -1);

	// Scroll bars
	CONFIGDLG(flag, xscroll, "xscroll", 700,
				rc.bottom - 100, rc.right - 750, 45);
	CONFIGDLG(flag, yscroll, "yscroll", rc.right - 60, 90, 45, rc.bottom - 200);
}


void CShadesDlg::OnExitaltResetdialogpositions() 
{
	P2N("Set / Reset Dialog positions %d\r\n");	
