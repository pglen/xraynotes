...

{
	...
	//shield.Create(IDD_HIPAA);
	//RepairPath(abook_root);
	//CString approot; GetAppRoot(approot);
	//dbfile = approot + 

	dbfile   = droot + "db\\address.sqlt";
	dbfile2  = droot + "db\\address_history.sqlt";
...

{
	...
	if (command != "")
		...
		if(strlen(str))
			...
			if(find)
				{
				filter.Format(	"where   (lname like '%%%s%%') or "
										"(fname like '%%%s%%') or "
										"(tel like '%%%s%%') or "
										"(email like '%%%s%%') or "
										"(city  like '%%%s%%') or "
				...
				{
				filter.Format("where lname like '%%%s%%' ",  str);
				}	
...

{
	...
	if(MBOX(tmpbox, "Delete prompt", MB_YESNO, IDNO) != IDYES)
		return ;
...
BOOL CAddrEntry::PreTranslateMessage(MSG* pMsg)
{
	...
	if(pMsg->message == WM_KEYDOWN)
		{
		// Capture ESC TAB ENTER
		if(pMsg->wParam == 9 || pMsg->wParam == 27 || pMsg->wParam == 13)
			{
			goto endd;
			}
